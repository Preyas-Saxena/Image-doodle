...
class datapairs_updated(Dataset):
...
# Writing function for retrieving random negative samples:
    def random_negative_samples(self, outfit_id):
        outfits = list(self.im2type)
        ...
        for i in range(ctr):
            ...
            candidate_items = self.im2type[random_outfit]
            random_item = random.choice(list(candidate_items))
            neg_samples.append((random_outfit, random_item))
    ...
    def __getitem__(self, index):
        ...
        random_neg_list= self.random_negative_samples(outfit_id)
        len_negs= len(neg_list)

        if len_negs<2:
            pending= 2-len_negs
            difference= [item for item in random_neg_list if item not in neg_list]
            for i in range(pending):
                random_sample = random.choice(difference)
                neg_list.append(random_sample)
                difference.remove(random_sample)
